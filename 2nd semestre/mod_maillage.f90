...
module mod_maillage
  ...
  contains
    ...
    subroutine maillage(fichier_maillage, nb_mailles, nb_aretes                        &
      ...
      !  il nous faut le code cl d'une arete de bord par son numÃ©ro global
      ! boucle aretes i
      !! quand un nÂ° triangle = 0, alors arete de bord
  ...
  !!! on pourrait donne la cl au triangle de bord
    ...
    subroutine connectivite(p,s,e,ar,trig)
      ...
      !1)-calcul du nombre d'arÃªtes nb_cotes
      nb_cotes=0
      allocate(flag(1:nb_noeuds,1:nb_noeuds))
      ! flag(n1,n2) contient en fin de boucle le numÃ©ro de l'arÃªte liant les sommets de numÃ©ros n1 et n2
      flag=0
      do i=1,nb_trig
      ...
      !2)-remplissage de e
      allocate(e(1:nb_cotes,2))    ! numÃ©ros des noeuds extrÃ©mitÃ©s de chaque arÃªte
      do i=1,nb_trig
         n1=S(i,1); n2=S(i,2); n3=S(i,3)
      ...
      allocate(ar(1:nb_trig,3))    ! numÃ©ros des arÃªtes de chaque triangle
      allocate(trig(1:nb_cotes,2)) ! numÃ©ro des triangles ayant l'arÃªte i en commun
      trig=0
      do i=1,nb_trig
         n1=S(i,1); n2=S(i,2); n3=S(i,3)
         !-arete (n1,n2)
